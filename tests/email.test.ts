...
describe("email notification engine", () => {
  ...
  it("renders direct request links as hyperlinks in HTML emails", async () => {
    ...
    const entries = await logLines(logPath);
    const studentEntry = entries.find((entry) => entry.event === "student.submission_created");
    expect(studentEntry?.html).toContain('href="http://localhost:5001/student/dashboard/sub-001"');
    expect(studentEntry?.html).toContain(">View request</a>");
  });
  ...
  it("uses saved admin SMTP settings ahead of environment defaults", async () => {
    ...
    process.env.EMAIL_CONFIG_SOURCE = "admin";
    process.env.EMAIL_DELIVERY_MODE = "log";
