...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await setCustomFormStatus(params.id, "archived", user, request.headers.get("x-forwarded-for") || undefined);
    return NextResponse.json({ form: updated });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Form could not be archived." }, { status: 400 });
  }
}
